...
def show_manager_menu():
    ...
    while True: 
        ...
        while command_number not in consts.MANAGER_SUPPORTED_COMMANDS.keys():
            print("ERROR: Command does not exist. Try again.")
            command_number = int(input("command > "))
        
        if command_number == consts.MGR_EXIT:
            stub.disconnect()
            return
...
def show_user_menu():
    ...
    while True:
        ...
        while command_number not in consts.USER_SUPPORTED_COMMANDS.keys():
            print("Command does not exist. Try again.")
            command_number = int(input("command > "))
        
        if command_number == consts.USER_EXIT:
            stub.disconnect()
            return
        
        request = user_command_to_request(command_number)
